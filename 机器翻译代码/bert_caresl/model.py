...
class Casrel(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.bert_dim = 768
        self.bert_encoder = BertModel.from_pretrained(config.model_path, config=self.config)
    ...
    def get_objs_for_specific_sub(self, sub_head_mapping, sub_tail_mapping, encoded_text):
        ...
        sub_head = torch.matmul(sub_head_mapping.float(), encoded_text)
        # [batch_size, 1, bert_dim]
        sub_tail = torch.matmul(sub_tail_mapping.float(), encoded_text)
        # [batch_size, 1, bert_dim]
